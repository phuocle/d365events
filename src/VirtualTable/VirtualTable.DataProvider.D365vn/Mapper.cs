...
namespace VirtualTable.DataProvider.D365vn
{
    public class Mapper
    {
        ...
        public Mapper(IPluginExecutionContext context, IOrganizationService service, ITracingService tracing)
        {
        ...
        public virtual object MapToVirtualEntityValue(AttributeMetadata entityAttribute, object value)
        {
            ...
            else if (entityAttribute.LogicalName == this.PrimaryEntityMetadata.PrimaryIdAttribute && int.TryParse(value.ToString(), out int keyInt))
            {
            ...
            else if (entityAttribute is LookupAttributeMetadata lookupAttr && int.TryParse(value.ToString(), out int lookupInt))
            {
